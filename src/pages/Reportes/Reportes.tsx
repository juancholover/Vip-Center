...
import { ReportesApi, ReporteIngresosDTO, ReporteComparativoDTO } from "../../api/reportesApi";
  ...
  return (
    <div className="min-h-screen bg-[#0A0E12] p-6 space-y-6">
      ...
      <motion.div
        ...
        className="bg-gradient-to-r from-emerald-600 via-teal-600 to-blue-600 rounded-2xl p-8 shadow-2xl"
      >
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-4xl font-bold text-white mb-2">📊 Reportes Avanzados</h1>
            <p className="text-emerald-100">Análisis completo del rendimiento de tu gimnasio</p>
          </div>
      ...
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="bg-[#1A1F25] rounded-xl p-2 shadow-xl border border-white/10"
      >
      ...
      >
        {tab === "overview" && (
          <div className="space-y-6">
            {/* Selector de Período */}
            <div className="bg-[#1A1F25] rounded-xl p-4 shadow-xl border border-white/10">
...
function ComparativaCard({ data }: ComparativaCardProps) {
  ...
  const valorAnterior = data?.valorPeriodoAnterior ?? 0;

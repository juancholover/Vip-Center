...
export default function AsistenciaModule() {
  ...
  useEffect(() => {
    const cargarEstadisticas = async () => {
      try {
        ...
        setPromedioDiario(stats.promedioDiario);
      } catch (error) {
  ...
  return (
    <div className="space-y-3">
      {/* Header y Botón para Escanear */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-[#1A1F25] p-3 rounded-lg border border-white/10 gap-2">
        <div>
          <h2 className="text-lg font-bold text-white">Panel de Asistencias</h2>
          <p className="text-sm text-slate-400">Verifica quiénes han ingresado hoy</p>
        </div>
        <button
          onClick={() => window.location.href = "/control-acceso"}
          className="flex items-center gap-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white px-4 py-2 rounded-lg font-medium transition shadow-lg shrink-0"
        >
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <path d="M14 14H17V20H14V14Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
      ...
      {/* Controles */}
      <div className="bg-[#1A1F25] rounded-lg border border-white/10 p-3">
...
function StatCard({
  ...
  return (
    <div
      className={`bg-gradient-to-br ${colorClasses[color]} rounded-lg border p-2.5`}
    >
